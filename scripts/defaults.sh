...
if [ ! -z "$CUSTOM_DOWNLOAD_DIR" ]; then
  read -p "We already set FreeNAS Download dir to [${CUSTOM_DOWNLOAD_DIR}]: " TMP_CUSTOM_DOWNLOAD_DIR
  TMP_CUSTOM_DOWNLOAD_DIR=${TMP_CUSTOM_DOWNLOAD_DIR:-$CUSTOM_DOWNLOAD_DIR}
else
  read -p "FreeNAS Download dir not set, default is [${DEFAULT_DOWNLOAD_DIR}]: " TMP_CUSTOM_DOWNLOAD_DIR
